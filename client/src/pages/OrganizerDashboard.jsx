...
function OrganizerDashboard() {
  ...
  useEffect(() => {
    if (!isAuthenticated || !isOrganizer) {
      navigate("/login"); // Redirect if not an authenticated organizer
      return;
    }
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      setNewEventData({ title: "", description: "", date: "", location: "" }); // Clear form
      setShowCreateForm(false);
      fetchOrganizerEvents(); // Refresh events
    } catch (error) {
        ...
        <ul>
          {pastEvents.map((event) => (
            <li key={event._id}>
              ...
              {/* Past events usually can't be edited/deleted, but you can enable if needed */}
              {/* <button onClick={() => handleEditClick(event)}>Edit</button> */}
              {/* <button onClick={() => handleDelete(event._id)}>Delete</button> */}
              <p>Attendees: {event.attendees.length}</p>
            </li>
